...
def job_event_handler(name):
    job = job_plantilla()
    job['metadata']['name'] = job['metadata']['name'] + '-' + name
    job['spec']['template']['spec']['containers'][0]['name'] = job['spec']['template']['spec']['containers'][0][
                                                                   'name'] + '-' + name
    job['spec']['template']['spec']['containers'][0]['env'][0]['value'] = name
    return job
...
def evento(mensaje, reason, nombre,
    ...
    estructura_evento = {
        ...
        'kind': 'Event',
        'message': mensaje, #Equivale al campo note 'Human readable description of the status of this operation.'
        'reason': reason,
        'type': 'Normal',
        'metadata': {
            'name': 'evento-' + nombre,
...
def customResourceEventObject(action, CR_type, CR_object, message, reason):
    ...
    if CR_type == "application":
             apiVersion = 'ehu.gcis.org/v1alpha4'
             kind = 'Application'
             # namespace = 'default'
    elif CR_type == "component":
             apiVersion = 'ehu.gcis.org/v1alpha1'
             kind = 'Component'
             # namespace = 'default'
    else:  # en este caso es el nivel generico
             apiVersion = 'ehu.gcis.org/v1alpha1'
             kind = str(CR_type).capitalize()
